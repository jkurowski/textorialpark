...
class PropertyService
{
    ...
    public function upload_2(string $title, UploadedFile $file, object $model, bool $delete = false)
    {

        if ($delete) {
            if (File::isFile(public_path('investment/property/' . $model->file_2))) {
                File::delete(public_path('investment/property/' . $model->file_2));
            }
            if (File::isFile(public_path('investment/property/thumbs/' . $model->file_2))) {
                File::delete(public_path('investment/property/thumbs/' . $model->file_2));
            }
            if (File::isFile(public_path('investment/property/list/' . $model->file_2))) {
                File::delete(public_path('investment/property/list/' . $model->file_2));
            }

            // WebP
            if (File::isFile(public_path('investment/property/webp/' . $model->file_2_webp))) {
                File::delete(public_path('investment/property/webp/' . $model->file_2_webp));
            }
            if (File::isFile(public_path('investment/property/thumbs/webp/' . $model->file_2_webp))) {
                File::delete(public_path('investment/property/thumbs/webp/' . $model->file_2_webp));
            }
            if (File::isFile(public_path('investment/property/list/webp/' . $model->file_2_webp))) {
                File::delete(public_path('investment/property/list/webp/' . $model->file_2_webp));
            }
        }

        $name = date('His').'_2_'.Str::slug($title).'.' . $file->getClientOriginalExtension();
        $name_webp = date('His').'_'.Str::slug($title).'.webp';
        $file->storeAs('property', $name, 'investment_uploads');
